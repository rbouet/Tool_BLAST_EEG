...
chan_ica.label   = {'Fp1', 'Fp2', 'F7', 'F3', 'F4', 'F8', 'Fz', 'C3', 'Cz', 'C4', 'P3', 'P4', 'Pz', 'O1', 'O2', 'EO+'};
...
switch GUI.Source
    ...
    case 'TRC_Blast'
        ...
        data_eog_OY_continue = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_eog, win_OY, double(GUI.BLAST_Object.Fs), 0, 20);
        data_eog_CY_continue = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_eog, win_CY, double(GUI.BLAST_Object.Fs), 0, 20);
        
        data_ica_OY_continue_raw = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_ica, win_OY, double(GUI.BLAST_Object.Fs), 0, 'none');
        data_ica_CY_continue_raw = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_ica, win_CY, double(GUI.BLAST_Object.Fs), 0, 'none');
        data_ica_OY_continue_filt = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_ica, win_OY, double(GUI.BLAST_Object.Fs), 0, 20);
        data_ica_CY_continue_filt = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_ica, win_CY, double(GUI.BLAST_Object.Fs), 0, 20);
        
        data_ica_OY_trial = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_ica, win_OY, double(GUI.BLAST_Object.Fs), 1, 'none');
        data_ica_CY_trial = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_ica, win_CY, double(GUI.BLAST_Object.Fs), 1, 'none');
    ...
    case 'MAT_Neurofeedback'
        ...
        data_ica_OY_continue_raw = Extract_datas_fromManu('OE', chan_ica, 0, 'none');
        data_ica_CY_continue_raw = Extract_datas_fromManu('CE', chan_ica, 0, 'none');
        data_ica_OY_continue_filt = Extract_datas_fromManu('OE', chan_ica, 0, 20);
        data_ica_CY_continue_filt = Extract_datas_fromManu('CE', chan_ica, 0, 20);
        
        data_ica_OY_trial = Extract_datas_fromManu('OE', chan_ica, 1, 'none'); 
        data_ica_CY_trial = Extract_datas_fromManu('CE', chan_ica, 1, 'none');
...
data_occi_OY_trial    = ft_selectdata(cfg, data_ica_OY_trial);

cfg         = [];
cfg.channel = chan_front.label;
data_front_CY_trial    = ft_selectdata(cfg, data_ica_CY_trial);
...
%% RTB 
% dimension reduction (channels average)
data_front_CY = Dimension_reduc(data_front_CY_trial);
data_front_OY = Dimension_reduc(data_front_OY_trial);
...
function data = Extract_datas_FT(filename, chan, win, Fs, epoked, lowpass)
...
function data = Extract_datas_fromManu(eyes, label, epoked, lowpass)
...
data.cfg.headerformat = 'matlab_manu';
data.cfg.dataformat = 'matlab_manu';
...
% data_occi_CY  = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_occi, win_CY, double(GUI.BLAST_Object.Fs), 0);
data_occi_CY = nanmean(data_occi_CY.trial{1},1);
% data_occi_OY  = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_occi, win_OY, double(GUI.BLAST_Object.Fs), 0);
...
fft_CY = [];
for xi_trial = 1 : numel(data_occi_CY.trial)
    
    data_t = nanmean(data_occi_CY.trial{xi_trial},1);
    fft_CY = [fft_CY; abs(fft(data_t, GUI.BLAST_Object.Fs))];

end, clear xi_trial

fft_OY = [];
for xi_trial = 1 : numel(data_occi_OY.trial)
    
    data_t = nanmean(data_occi_OY.trial{xi_trial},1);
    fft_OY = [fft_OY; abs(fft(data_t, GUI.BLAST_Object.Fs))];

end, clear xi_trial
fft_CY = mean(fft_CY,1);
fft_OY = mean(fft_OY,1);
...
figure('Position', [100,100, 500 500],  'MenuBar', 'no', 'Color', GUI.Colors(1,:),...
       'Tag', 'Frequency_power');
f_p = plot(x,fft_CY(x), '--b')
f_p.Parent.YLim = [min(fft_CY) median(fft_CY) + max(fft_CY(4:end))];
hold on, plot(x,fft_OY(x), '--k')
plot(x,fft_diff(x), 'r')
plot([alpha_peak alpha_peak], [min(fft_diff*1.1) max(fft_diff*1.3)], '-g')
legend({'Eyes Close', 'Eyes Open', 'Difference', 'Alpha Pic'})
xlabel('Hz')
...
function data = artefact_eog(data_eog, data_ica, data_to_correct, corr_eog_th, titre)
...
data_front_OY = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_front, win_OY, double(GUI.BLAST_Object.Fs), 0, 'no');
data_front_CY = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_front, win_CY, double(GUI.BLAST_Object.Fs), 0, 'no');
temp = chan_eog; chan_eog = {}; chan_eog.label = temp;
clear temp
data_eog_CY   = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_eog, win_CY, double(GUI.BLAST_Object.Fs), 0, 10);
data_eog_OY   = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_eog, win_OY, double(GUI.BLAST_Object.Fs), 0, 10);
...
data_front_OY = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_front, win_OY, double(GUI.BLAST_Object.Fs), 0, 45);
data_front_CY = Extract_datas_FT(GUI.BLAST_Object.Path_TRC, chan_front, win_CY, double(GUI.BLAST_Object.Fs), 0, 45);
...
switch GUI.Source
 ...
 case 'MAT_Neurofeedback'
        ...
        data_front_OY = Extract_datas_fromManu('OE', chan_front, 0, 45);
        data_front_CY = Extract_datas_fromManu('CE', chan_front, 0, 45);
    
        data_eog_CY.time{1}  = data_front_CY.time{1};
        data_eog_CY.trial{1} = zeros(1,length(data_front_CY.time{1}));
        data_eog_OY.time{1}  = data_front_OY.time{1};
        data_eog_OY.trial{1} = zeros(1,length(data_front_OY.time{1}));
end
